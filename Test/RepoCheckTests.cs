...
public class RepoCheckTests {
  ...
  private Task ExecuteRunAsync(int installationId, string owner,
    ...
    var compressImagesMessage = new CompressImagesMessage{
        CloneUrl = cloneUrl, Owner = owner, InstallationId = installationId,
        RepoName = repoName};

    return ExecuteRunAsync(compressImagesMessage, prId, out logger);
  }

  private Task ExecuteRunAsync(CompressImagesMessage compressImagesMessage,
                               long prId, out ILogger logger) {
    ...
    return CompressImagesFunction.CompressImagesFunction.RunAsync(
        installationTokenProvider, compressImagesMessage, openPrMessages,
        settingsTable, repoChecks, logger, context);
  }
}
}
